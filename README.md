...
| **M03** | 需求分析 | 6类遗漏识别，三层多轮追问，场景地图双视图 | `已实现` |
| **M04** | 测试点分析 & 场景地图确认 | AI生成测试点草稿，粒度校验，用户评审确认，与场景地图统一 | `已实现` |
| **M05** | 对话式生成工作台 | 三栏布局，4种生成模式，流式输出 | `已实现` |
...
| ~~M18~~ | ~~协作功能~~ | ~~评论、@提及~~ | `已裁剪` |
| **M19** | 首页仪表盘 | 项目概览 + 质量分析双Tab，柱状图统计 | `已实现` |
