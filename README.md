...
[![Version](https://img.shields.io/badge/version-v1.3.0-blue.svg)](https://github.com/koco-co/Sisyphus-case-platform)
...
> 💡 **核心理念**：测试用例生成分为两个本质不同的决策：**「测什么」**（测试点/场景地图）和**「怎么测」**（用例步骤）。Sisyphus-case-platform 显式拆分这两个阶段，通过「需求健康诊断 → 测试点分析确认 → 约束式用例生成 → 变更影响追踪 → 执行回流」形成持续进化的质量飞轮。

<br/>

[📖 产品概述](#-产品概述) · [🗺 核心流程](#-核心生成流程) · [✨ 健康诊断](#-需求健康诊断系统) · [🎯 测试点分析](#-测试点分析--场景地图确认) · [📄 需求Diff](#-需求文档-diff--变更影响分析) · [🚀 快速开始](#-快速开始)
...
Sisyphus-case-platform 是一款面向**企业数据中台场景**的 AI 功能测试用例智能生成平台，支持子产品多迭代并行研发模式，覆盖从需求录入、健康诊断、测试点分析、用例生成、变更追踪到执行回流的完整测试生命周期。
...
| 模块 | 名称 | 核心能力 | 优先级 |
|---|---|---|---|
| **M00** | 子产品/迭代/需求管理 | 三级层级结构，迭代周期管理，需求卡片编辑器 | `P0` |
| **M01** | 文档解析引擎（UDA层） | 任意格式→内部AST，外链图片归档，统一结构化 | `P0` |
| **M02** | 历史数据导入与清洗 | CSV/MD批量导入，AI清洗+人工审核，导入健康报告 | `P0` |
| **M03** | 需求健康诊断 | 6类遗漏识别，三层多轮追问，场景地图双视图 | `P0` |
| **M04** | 测试点分析 & 场景地图确认 | AI生成测试点草稿，粒度校验，用户评审确认，与场景地图统一 | `P0` |
| **M05** | 对话式生成工作台 | 三栏布局，4种生成模式，流式输出 | `P0` |
| **M06** | 用例管理中心 | CRUD、版本管理、评审流程、全链路溯源 | `P0` |
| **M07** | 需求文档 Diff & 影响分析 | 版本对比，变更范围识别，受影响用例自动标记 | `P0` |
| **M08** | 需求覆盖度矩阵 | 需求×用例二维视图，一眼发现覆盖空洞 | `P1` |
| **M09** | 迭代测试计划生成 | 基于迭代需求自动草拟计划，工作量预估 | `P1` |
| **M10** | 用例模板库 | 7类数据中台专用模板，企业自定义 | `P1` |
| **M11** | 知识库管理 | 企业规范向量化，RAG检索，命中率测试 | `P1` |
| **M12** | 用例导出与集成 | 任意格式导出（MD/Word/PDF/Excel/XMind/Jira） | `P1` |
| **M13** | 执行结果回流 | 失败/删除/高质量用例标记，驱动质量持续提升 | `P2` |
| **M14** | 质量分析看板 | 生成质量评分，覆盖度趋势，跨迭代对比 | `P2` |
| **M15** | 权限与安全管理 | RBAC，SSO，审计日志，数据隔离 | `P1` |
...
│                                                                     │
│  ❓ 待确认（2个）       [健康诊断追问后新增]                           │
│  ├─ [?] 并发修改同一数据源配置时的冲突处理（来自健康诊断）              │
│  └─ [?] 数据源删除时正在执行的同步任务处理策略（来自健康诊断）          │
│                                                                     │
│  [＋ 手动添加测试点]                                                  │
│                                                                     │
...
## 风险提示
- ⚠️ REQ-090 健康诊断发现 3 处遗漏，测试点尚未完全确认
...
| 技术 | 版本 | 用途 |
|---|---|---|
| **Next.js** (App Router) | 14+ | 主框架 SSR/SSG |
| **Ant Design Pro** | 6+ | 企业级 UI，ProTable/ProForm |
| **Zustand** + React Query | latest | 全局状态 + 服务端状态缓存 |
...
| 技术 | 版本 | 用途 |
|---|---|---|
| **FastAPI** | 0.110+ | 主框架，异步 API + SSE 流式推送 |
| **SQLAlchemy 2.0** + Alembic | 2+ | ORM + 数据库迁移 |
...
| **GPT-4o** / Claude 3.5 Sonnet | 主力生成模型（可切换） |
...
MVP v0.1 (第1-4周)
├── ✅ 用户认证、子产品/迭代/需求三级结构
├── ✅ Requirement Card 编辑器（TipTap，MD底层）
...
├── ✅ 历史数据导入 + 导入健康报告
├── ✅ 三栏工作台（流式生成 + 四种模式）
├── ✅ 用例 CRUD + 多格式导出（基于 UDA）
└── 🎯 验收：历史数据导入可用，工作台流畅

Beta v0.3 (第9-13周)  ← 诊断 + 测试点分析 MVP
├── 🆕 需求健康报告（6类遗漏识别）
├── 🆕 场景补全对话（广度扫描 + 深度聚焦）
├── 🆕 测试点分析页（AI生成草稿 + 用户确认 + 列表视图）
├── 🆕 测试点驱动生成模式
├── ✅ 知识库 RAG 集成
└── 🎯 验收：测试点生成准确率 > 80%，用户确认流程顺畅

RC v0.4 (第14-17周)  ← Diff + 场景地图可视化
├── 🆕 需求文档 Diff 引擎（含测试点影响分析）
├── 🆕 用例/测试点影响自动标记 + 通知推送
├── 🆕 场景地图树形可视化（React Flow）
├── 🆕 三层多轮追问完整实现（含交叉验证）
├── 🆕 行业必问清单（32条）内置
├── ✅ 图片 OCR（PaddleOCR + GPT-4 Vision）
├── ✅ 多模型切换 + 私有化部署
└── 🎯 验收：Diff 影响识别准确率 > 85%，可视化地图交互流畅

GA v1.0 (第18-23周)
├── 🆕 需求覆盖度矩阵（含测试点维度）
├── 🆕 迭代测试计划自动生成
├── 🆕 执行结果回流 + 质量飞轮（含测试点修改记录）
├── ✅ Jira / Xray 集成（含结果回写）
├── ✅ SSO 集成（LDAP/OAuth2）
├── ✅ 200 并发压测通过
├── ✅ 安全加固 + 审计日志
...
- Docker & Docker Compose 2.0+  |  Node.js 18+  |  Python 3.11+
...
python scripts/seed.py   # 初始化模板(7类) + 必问清单(32条) + 示例子产品
